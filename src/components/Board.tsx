...
export const Board = ({ pokemons, loading }: BoardProps) => {
  const [pokemonsProps, setPokemonsProps] = useState<Array<pokemonsPropsTypes>>(
    pokemons
  );
  ...
  useEffect((): void => {
    setTimeout(() => {
      ...
      pokemonsProps.forEach((pokemon: any) => {
        setTimeout(
          () => setCardIsFlipped(pokemon.id, true, true),
          index++ * 100
        );
      });
  ...
  useEffect(() => {
    {
      firstCard.idPokemon !== secondCard.idPokemon
        ? setColorBoard(s.red)
        : setColorBoard(s.green);
    }
  ...
  const Cards = pokemonsProps.map((pokemon: any) => (
    <Card
  ...
  return (
    <div
      className={
        s.boardContainer +
        (canClickBoard ? "" : ` ${s.blockBoard} ${colorBoard}`)
      }
    >
      {Cards}
    </div>
